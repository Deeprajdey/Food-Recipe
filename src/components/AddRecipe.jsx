...
import { addRecipeData } from "../store/reducers/recipeSlice";
...
const AddRecipe = () => {
  ...
  const [btnInfo, setBtnInfo] = useState("Add Recipe");
  ...
  const handleChangeIngredientData1 = (e) => {
    setFormData((formData) => {
      ...
      ingredientData.ingredients[e.target.name].ingredient = e.target.value;
      return {
        ...ingredientData,
      };
    });
  ...
  const handleSubmit = (e) => {
    ...
    if (Number(formData.ingredientsNum) === 0) {
      setFormValidation((formValidation) => ({
        ...formValidation,
